...
public class Search
{
	...
	public static boolean recursiveBinarySearch( int[] A, int p, int r, int x )
	{
		...
		if( p <= r )
		{
			int q = ( p + r ) / 2;
			if( A[ q ] < x )
			{  // A[q] is too small, x is not in left half
			...
			{ // A[q] > x, so x is not in the right half
	...
	/**
	 * Implementation of Binary cs6301.g00.Search algorithm using generics.  Use on any
	...
	/**
	 * Implementation of Binary cs6301.g00.Search algorithm using generics.  This can only be used on a
	 * type that implements Comparable itself.  cs6301.g00.Item class can use it, but ItemExt will not.
	 */
	...
	/**
	 * Implementation of Binary cs6301.g00.Search algorithm using generics.  Use on any type that
	...
	/* cs6301.g00.Search for many random elements to find time taken */
